...
async function loadEpub(filePath) {
    ...
    // Extract spine
    const spine = pkg.spine[0].itemref.map((item) => ({
        idref: item.$.idref,
        linear: item.$.linear,
    }));
    // Try to extract TOC
...
function extractMetadata(metadataNode) {
    ...
    const dc = (key) => getText(metadataNode[`dc:${key}`]) || getText(metadataNode[key]);
    ...
    if (Array.isArray(subjects)) {
        metadata.subject = subjects.map((s) => typeof s === "string" ? s : s._ || String(s));
    }
    return metadata;
}
async function extractToc(zip, manifest, basePath, pkg) {
    const toc = [];
    // Try EPUB 3 nav document first
    for (const [, item] of manifest) {
        if (item.mediaType === "application/xhtml+xml") {
            const fullPath = basePath === "." ? item.href : `${basePath}/${item.href}`;
    ...
    if (tocId && manifest.has(tocId)) {
        ...
        const ncxPath = basePath === "." ? ncxItem.href : `${basePath}/${ncxItem.href}`;
    ...
    for (const [, item] of manifest) {
        if (item.href.endsWith(".ncx")) {
            const ncxPath = basePath === "." ? item.href : `${basePath}/${item.href}`;
            const ncxContent = await zip.file(ncxPath)?.async("text");
            if (ncxContent) {
                return await parseNcxToc(ncxContent);
            }
...
async function parseNavToc(navContent) {
    const toc = [];
...
async function parseNcxToc(ncxContent) {
...
function parseNavPoints(navPoints) {
    return navPoints.map((point) => {
        ...
        if (point.navPoint && Array.isArray(point.navPoint)) {
            item.children = parseNavPoints(point.navPoint);
        }
        return item;
    });
}
async function getChapterContent(epub, index) {
    if (index < 0 || index >= epub.spine.length) {
        throw new Error(`Chapter index ${index + 1} out of range. Book has ${epub.spine.length} chapters.`);
    }
    ...
    const fullPath = epub.contentBasePath === "."
        ? manifestItem.href
...
async function searchContent(epub, query) {
    const results = [];
